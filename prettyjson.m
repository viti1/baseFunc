function [output_text] = prettyjson(input_struct,filename)
...
% gets confused and !!mangles!! JSON when strings contain [ ] { or }. 
    ...
    output_text = strjoin(lines, newline);
    output_text = strrep(output_text,'\n','\r\n');
    
    if exist('filename','var')
        if exist(filename,'file') == 7 % its a folder
            filename = [ filename filesep 'parameters.json' ];
        end
